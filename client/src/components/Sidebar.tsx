...
const Sidebar = ({
  ...
  const navItems = [
    ...
    { to: '/volunteers', icon: BookOpen, label: t('nav.volunteers'), permission: PERMISSIONS.volunteersView },
    { to: '/members', icon: ShieldCheck, label: t('nav.members'), permission: PERMISSIONS.membersView },
    ...
    >
      <div className={`${collapsed ? 'p-4' : 'p-6'} border-b border-slate-800/70`}>
        <div
          ...
          title={collapsed ? `${branding.shortName} Gestionale - ${branding.organizationName}` : undefined}
        >
          <div className={`flex items-center gap-3 ${collapsed ? 'justify-center' : 'min-w-0 flex-1'}`}>
            ...
            {!collapsed && (
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-500 break-words">{branding.organizationName}</p>
              </div>
            )}
          </div>
          {!collapsed && (

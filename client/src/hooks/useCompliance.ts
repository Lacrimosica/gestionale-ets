...
export interface ComplianceRules {
  documentTypes: Record<string, {
    label: string;
    description: string;
    // New: explicit list of consent type keys (e.g. ['third_party', 'image_use'])
    consentTypes?: string[];
    /** @deprecated use consentTypes — kept for backwards compat with old saved rules */
    hasConsents?: boolean;
    versions?: string[];
...
export interface ComplianceDocument {
  ...
  // Dynamic consent state keyed by consent type (e.g. 'third_party', 'image_use')
  consentsByType: Record<string, ConsentEntry>;
  notes?: string | null;
  createdAt: string;
  updatedAt: string;
...
export const useCompliance = (personId?: string, enabled = true) => {
  ...
  const [rules, setRules] = useState<ComplianceRules>({ documentTypes: {}, roles: {}, baseRequirements: { isVolunteer: [], isSocio: [], isBoard: [] } });
  ...
  const recordConsent = async (data: {
    personId: string;
    documentId?: string;
    consentType: string;
    status: 'granted' | 'withdrawn' | 'pending';
    ...
    await axios.post(`${API_BASE_URL}/compliance/consent-records`, data);
    await refresh();
  };

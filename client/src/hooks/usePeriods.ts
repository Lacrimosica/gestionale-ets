...
export const usePeriods = (personId?: string) => {
  ...
  const endVolunteering = async (periodId: string, data: { exitDate: string; exitReason: string }) => {
    try {
      setLoading(true);
      await axios.patch(`${API_BASE_URL}/periods/volunteers/${periodId}`, {
        ...data,
        status: 'resigned'
      });
    } catch (err: any) {
  ...
  const updateVolunteerPeriod = async (periodId: string, data: Partial<VolunteerPeriod>) => {
    try {
      setLoading(true);
      await axios.patch(`${API_BASE_URL}/periods/volunteers/${periodId}`, data);
    } catch (err: any) {
      throw new Error(err.response?.data?.error || 'Error');
    } finally {
      setLoading(false);
  ...
  const deleteVolunteerPeriod = async (periodId: string) => {
    try {
      setLoading(true);
      await axios.delete(`${API_BASE_URL}/periods/volunteers/${periodId}`);
    } catch (err: any) {
      throw new Error(err.response?.data?.error || 'Error');
    } finally {
      setLoading(false);
  ...
  const endMembership = async (periodId: string, data: { resignationDate: string; exitReason?: string }) => {
    try {
      setLoading(true);
      await axios.patch(`${API_BASE_URL}/periods/members/${periodId}`, data);
    } catch (err: any) {
      throw new Error(err.response?.data?.error || 'Error');
    } finally {
      setLoading(false);
  ...
  const updateMemberPeriod = async (periodId: string, data: Partial<MemberPeriod>) => {
    try {
      setLoading(true);
      await axios.patch(`${API_BASE_URL}/periods/members/${periodId}`, data);
    } catch (err: any) {
  ...
  const deleteMemberPeriod = async (periodId: string) => {
    try {
      setLoading(true);
      await axios.delete(`${API_BASE_URL}/periods/members/${periodId}`);
    } catch (err: any) {
      throw new Error(err.response?.data?.error || 'Error');
    } finally {
      setLoading(false);

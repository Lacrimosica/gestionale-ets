...
import {
  ...
  FileCode, FileDown, Copy, Check, Users, Info, Edit3, ListOrdered
} from 'lucide-react';
...
const TimelineView = () => {
  ...
  const timelineEvents = data.events?.filter((d: TimelineEntry) => ['assembly', 'member_admission', 'member_resignation'].includes(d.type)) ?? [];
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700 w-full max-w-[1600px] mx-auto pb-10">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl relative overflow-hidden h-[calc(100vh-220px)] flex flex-col">
        <aside className="border-b border-slate-800 bg-slate-950/80 overflow-hidden shrink-0 max-h-[42%]">
          {selectedEntry ? (
            <div className="p-4 overflow-y-auto space-y-4">
              <div className="flex items-center justify-between gap-2">
                <h3 className="text-sm font-black text-slate-300 uppercase tracking-wider">{t('timeline.eventDetail')}</h3>
                <button type="button" onClick={() => setSelectedId(null)} className="p-1.5 rounded-lg hover:bg-slate-800 text-slate-400 hover:text-white transition-colors" title={t('common.close', { defaultValue: 'Close' })}>âœ•</button>
              </div>
                ...
                return members.length > 0 ? (
                  <div className="pt-3 border-t border-slate-800">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1.5">
                      {members.slice(0, 12).map((mem: MembershipHistory) => <div key={mem.id} className="text-xs text-slate-300 truncate font-medium bg-slate-900/40 px-2 py-1 rounded border border-slate-800/30">{mem.firstName} {mem.lastName}</div>)}
                    </div>
                    {members.length > 12 && (
        ...
        >
          <div className="relative py-12 min-w-[1000px]">
            ...
            {(() => {
              ...
              const timelineEntries = events.filter((d: TimelineEntry) => ['assembly', 'member_admission', 'member_resignation'].includes(d.type));
                        ...
                        <div key={dayStr} className="absolute left-[180px] flex items-start z-20 hover:z-[70]" style={{ top: `${groupY}px` }}>
                          ...
                          <div className="flex flex-wrap gap-3 max-w-[1000px]">
                            {group.map(entry => {
                              ...
                              const colorClass = isAssembly ? 'bg-purple-600/10 text-purple-400 border-purple-500/30' : isAmm ? 'bg-green-600/10 text-green-400 border-green-500/30' : 'bg-red-600/10 text-red-400 border-red-500/30';
                              let dLabel = '';
                              let snapshotCount = 0;

                              if (isAssembly) {
                                snapshotCount = getHistoricalMembersAtDate(entry.start).length;
                                // const conv = entry.tenuta === 'seconda' ? '2Â°' : '1Â°';
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex items-center space-x-2 px-3 py-2 rounded-lg border shadow-lg backdrop-blur-md transition-all hover:scale-[1.02] hover:brightness-110 cursor-pointer text-left ${colorClass} ${selectedId === entry.id ? 'ring-2 ring-blue-400 ring-offset-2 ring-offset-slate-900' : ''}`}
                                >
                                  {isAssembly ? <FileText size={14} /> : isAmm ? <UserPlus size={14} /> : <UserMinus size={14} />}
        ...
        <div className="bg-slate-900 border-t border-slate-800 py-3 px-8 flex justify-center space-x-8 shrink-0">

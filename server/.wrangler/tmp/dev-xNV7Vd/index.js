...
// .wrangler/tmp/bundle-NlQ7xS/checked-fetch.js
...
// .wrangler/tmp/bundle-NlQ7xS/strip-cf-connecting-ip-header.js
...
// src/db/schema.ts
var persone = sqliteTable("persone", {
  ...
  paeseNascita: text("paese_nascita"),
  genere: text("genere"),
...
var periodiVolontario = sqliteTable("periodi_volontario", {
  id: text("id").primaryKey(),
  personaId: text("persona_id").notNull().references(() => persone.id),
  status: text("status").notNull(),
  // 'attivo' | 'inattivo' | 'sospeso' | 'dimesso'
  dataIscrizione: text("data_iscrizione").notNull(),
  dataUscita: text("data_uscita"),
  motivoUscita: text("motivo_uscita"),
  note: text("note"),
  createdAt: text("created_at").notNull()
});
var periodiSocio = sqliteTable("periodi_socio", {
  id: text("id").primaryKey(),
  personaId: text("persona_id").notNull().references(() => persone.id),
  periodoVolontarioId: text("periodo_volontario_id").notNull().references(() => periodiVolontario.id),
  dataAmmissione: text("data_ammissione").notNull(),
  dataDimissione: text("data_dimissione"),
  motivoUscita: text("motivo_uscita"),
  riferimentoArt: text("riferimento_art"),
  assembleaAmmissioneId: text("assemblea_ammissione_id").references(() => assemblee.id),
  assembleaUscitaId: text("assemblea_uscita_id").references(() => assemblee.id),
  note: text("note"),
  createdAt: text("created_at").notNull()
});
var assemblee = sqliteTable("assemblee", {
  id: text("id").primaryKey(),
  tipo: text("tipo").notNull(),
  // 'ordinaria' | 'straordinaria' | 'consiglio_direttivo'
  numero: integer("numero").notNull(),
  dataConvocazione: text("data_convocazione").notNull(),
  dataPrimaConv: text("data_prima_conv").notNull(),
  oraPrimaConv: text("ora_prima_conv").notNull(),
  dataSecondaConv: text("data_seconda_conv"),
  oraSecondaConv: text("ora_seconda_conv"),
  sede: text("sede").notNull(),
  modalita: text("modalita").notNull(),
  // 'presenza' | 'telematica' | 'mista'
  assembleaTenuta: text("assemblea_tenuta"),
  // 'prima' | 'seconda' | 'nessuna'
  presidente: text("presidente").notNull(),
  segretario: text("segretario").notNull(),
  note: text("note"),
  createdAt: text("created_at").notNull(),
  updatedAt: text("updated_at").notNull()
});
var ordiniDelGiorno = sqliteTable("ordini_del_giorno", {
  id: text("id").primaryKey(),
  assembleaId: text("assemblea_id").notNull().references(() => assemblee.id),
  numero: integer("numero").notNull(),
  titolo: text("titolo").notNull(),
  descrizione: text("descrizione"),
  delibera: text("delibera"),
  createdAt: text("created_at").notNull()
});
var presenze = sqliteTable("presenze", {
  id: text("id").primaryKey(),
  assembleaId: text("assemblea_id").notNull().references(() => assemblee.id),
  personaId: text("persona_id").notNull().references(() => persone.id),
  modalita: text("modalita").notNull(),
  // 'presenza' | 'telematica' | 'delega'
  deleganteId: text("delegante_id").references(() => persone.id),
  createdAt: text("created_at").notNull()
});
...
var MySqlTable = class extends Table {
  /** @internal */
  [(_a88 = entityKind, Table.Symbol.Columns)];
  /** @internal */
  [InlineForeignKeys3] = [];
  /** @internal */
  [Table.Symbol.ExtraConfigBuilder] = void 0;
...
__publicField(MySqlTable, _a88, "MySqlTable");
/** @internal */
...
_a89 = entityKind;
__publicField(ForeignKeyBuilder3, _a89, "MySqlForeignKeyBuilder");
var _a90;
...
_a90 = entityKind;
__publicField(ForeignKey3, _a90, "MySqlForeignKey");
...
_a91 = entityKind;
__publicField(UniqueConstraintBuilder3, _a91, "MySqlUniqueConstraintBuilder");
var _a92;
...
_a92 = entityKind;
__publicField(UniqueOnConstraintBuilder3, _a92, "MySqlUniqueOnConstraintBuilder");
var _a93;
...
_a93 = entityKind;
__publicField(UniqueConstraint3, _a93, "MySqlUniqueConstraint");
...
_a94 = entityKind;
__publicField(MySqlColumnBuilder, _a94, "MySqlColumnBuilder");
var _a95;
...
_a95 = entityKind;
__publicField(MySqlColumn, _a95, "MySqlColumn");
var _a96;
...
_a96 = entityKind;
__publicField(MySqlColumnBuilderWithAutoIncrement, _a96, "MySqlColumnBuilderWithAutoIncrement");
var _a97;
...
_a97 = entityKind;
__publicField(MySqlColumnWithAutoIncrement, _a97, "MySqlColumnWithAutoIncrement");
...
_a98 = entityKind;
__publicField(MySqlCharBuilder, _a98, "MySqlCharBuilder");
var _a99;
...
_a99 = entityKind;
__publicField(MySqlChar, _a99, "MySqlChar");
...
_a100 = entityKind;
__publicField(MySqlVarBinaryBuilder, _a100, "MySqlVarBinaryBuilder");
var _a101;
...
_a101 = entityKind;
__publicField(MySqlVarBinary, _a101, "MySqlVarBinary");
...
_a102 = entityKind;
__publicField(MySqlVarCharBuilder, _a102, "MySqlVarCharBuilder");
var _a103;
...
_a103 = entityKind;
__publicField(MySqlVarChar, _a103, "MySqlVarChar");
...
function p(e) {
  ...
  if (function(e2) {
    ...
    if (is(e, PgUUID))
      ...
      m2 = external_exports.array(p(e.baseColumn));
...
// src/routes/persone.ts
var personeRouter = new Hono2();
var insertPersonaSchema = c(persone);
var patchPersonaSchema = insertPersonaSchema.partial();
personeRouter.get("/", async (c2) => {
  const db = drizzle(c2.env.DB);
  const result = await db.select().from(persone).all();
  return c2.json(result);
});
personeRouter.get("/:id", async (c2) => {
  const id = c2.req.param("id");
  const db = drizzle(c2.env.DB);
  const result = await db.select().from(persone).where(eq(persone.id, id)).get();
  if (!result) {
    return c2.json({ error: "Persona non trovata" }, 404);
  }
  return c2.json(result);
});
personeRouter.post("/", async (c2) => {
  const body = await c2.req.json();
  const db = drizzle(c2.env.DB);
  const validation = insertPersonaSchema.safeParse({
    ...body,
  ...
  if (!validation.success) {
    return c2.json({ error: "Dati non validi", details: validation.error.format() }, 400);
  }
  const result = await db.insert(persone).values(validation.data).returning().get();
  return c2.json(result, 201);
});
personeRouter.patch("/:id", async (c2) => {
  const id = c2.req.param("id");
  const body = await c2.req.json();
  const db = drizzle(c2.env.DB);
  const validation = patchPersonaSchema.safeParse({
    ...body,
    updatedAt: (/* @__PURE__ */ new Date()).toISOString()
  });
  if (!validation.success) {
  ...
  const result = await db.update(persone).set(validation.data).where(eq(persone.id, id)).returning().get();
  if (!result) {
    return c2.json({ error: "Persona non trovata" }, 404);
  }
  return c2.json(result);
...
app.route("/api/persone", personeRouter);
var src_default = app;
...
// .wrangler/tmp/bundle-NlQ7xS/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-NlQ7xS/middleware-loader.entry.ts
